"""
Graph Completion Conflict Retriever
===================================

Specialized retriever for handling conflict-related queries with cross-nodeset conflict detection.
This retriever focuses on finding and analyzing conflicts between rule entities across different nodesets.
"""
...
class GraphCompletionConflictRetriever(GraphCompletionRetriever):
    """
    Specialized retriever for conflict detection and analysis.
    
    This class extends GraphCompletionRetriever to provide enhanced conflict detection
    capabilities, specifically for cross-nodeset rule conflicts.
    
    Public methods:
    - get_context: Enhanced with conflict detection logic
    - get_completion: Inherits from parent class
    """
    ...
    def __init__(
        ...
        """Initialize conflict retriever with prompt paths and search parameters."""
        super().__init__(
    ...
    async def get_context(self, query: str) -> str:
        """
        Enhanced context retrieval with conflict detection.

        Always performs cross-nodeset conflict analysis when node_name is provided.
        This ensures complete flow to fetch all rules in the given nodeset and
        their conflicts in other nodesets.

        Parameters:
        -----------
        query (str): The query string used to retrieve context

        Returns:
        --------
        str: Context string with conflict analysis or standard graph context
        """
        ...
        # Always perform conflict detection when node_name is provided
        # This ensures we fetch all rules and their cross-nodeset conflicts
        if self.node_name:
